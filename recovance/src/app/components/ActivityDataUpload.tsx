...
export default function ActivityDataUpload() {
  ...
  const handleUpload = async () => {
    if (!activitiesFile) return;

    setUploading(true);
    setResult(null);

    try {
      const content = await activitiesFile.text();
      const csvData = parseCSV(content);

      console.log("Parsed activities CSV data:", csvData);

      const response = await fetch("/api/garmin/upload-activities", {
        method: "POST",
        headers: {
      ...
      console.error("Error uploading activities data:", error);
      setResult({
        success: false,
        message: "Failed to upload activities data",
      });
    } finally {
      setUploading(false);
    }
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="flex-1">
          <input
            type="file"
            accept=".csv"
            onChange={(e) => setActivitiesFile(e.target.files?.[0] || null)}
          ...
          <p className="text-xs text-gray-400 mt-1">
            CSV format: Activity Type, Date, Favorite, Title, Distance,
            Calories, etc.
          </p>
        </div>
        <button
          onClick={handleUpload}
          disabled={!activitiesFile || uploading}
